...
class LSTM_ECG(Network_base):
    ...
    def create_placeholder(self):
        ...
        X = tf.placeholder(tf.float32, [None, self.time_step, self.n_channels], name='X')
        # Y.shape =======> (number of samples, n_classes)
        y_true = tf.placeholder(tf.float32, [None, self.n_classes], name='y_true')
        #batch_size = tf.placeholder(tf.int32, [])
        keep_prob = tf.placeholder(tf.float32, name='keep_prob')
        #x_lengths = tf.placeholder(tf.float32, [None], name='x_lengths')

        return X, y_true, keep_prob    #, x_lengths
    ...
    def train(self, X_tr, y_tr, X_valid = None, y_valid = None):
        ...
        outputs, final_state = tf.nn.dynamic_rnn(cell = multi_cell,
                                           inputs = X_in,
                                           dtype = tf.float32)   # sequence_length=x_lengths,  initial_state=init_state,

        with tf.name_scope("average_features"):
            average_output = tf.reduce_mean(outputs, axis = 1)

        pred = tf.matmul(average_output, weights['out']) + biases['out']
        ...
        for e in range(self.training_epochs):
            ...
            f1 = self.compute_f1_score(train_pred, train_labels)
            train_f1, f1_AF, f1_block, f1_PC, f1_ST = f1

            epoch_end = time()
            print("Epoch  {}/{}:".format(e, self.training_epochs),
                  "train_loss:{:.4f},".format(np.mean(epoch_losses)),
                  "train_acc:{:.4f},".format(np.mean(epoch_acc)),
                  "overall_f1:{:.4f},".format(train_f1),
                  "f1_AF:{:.4f},".format(f1_AF),
                  "f1_block:{:.4f},".format(f1_block),
                  "f1_PC:{:.4f},".format(f1_PC),
                  "f1_ST:{:.4f},".format(f1_ST),
            ...
            if (X_valid is not None) and (y_valid is not None):
                ...
                for  x_t, y_t in valid_mini_batches:
                    feed = {X: x_t,
                            y_true: y_t,
                            keep_prob: 1.0}    # , x_lengths:lengths, batch_times: batch_maxtime
                ...
                f1 = self.compute_f1_score(valid_pred, valid_labels)
                valid_f1, f1_AF, f1_block, f1_PC, f1_ST = f1
                valid_end = time()
                print("Epoch  {}/{}:".format(e, self.training_epochs),
                      "valid_loss:{:.4f},".format(np.mean(epoch_vlosses)),
                      "valid_acc:{:.4f},".format(np.mean(epoch_vacc)),
                      "overall_f1:{:.4f},".format(valid_f1),
                      "f1_AF:{:.4f},".format(f1_AF),
                      "f1_block:{:.4f},".format(f1_block),
                      "f1_PC:{:.4f},".format(f1_PC),
                      "f1_ST:{:.4f},".format(f1_ST),
                ...
                if np.mean(epoch_vacc) > max_acc:   # 保存测试集正确率最大的模型
                    max_acc = np.mean(epoch_vacc)
                    saver.save(self.sess, "./checkpoint_dir/lstm", global_step=e+1)
            if self.save_result:
                np.savetxt('./result/train_pred.txt', train_pred, fmt="%d", delimiter=" ")
                if (X_valid is not None) and (y_valid is not None):
                    np.savetxt('./result/valid_pred.txt', valid_pred, fmt="%d", delimiter=" ")

        saver.save(self.sess, "./checkpoint_dir/lstm")   #保存最后一个epoch的模型

        if self.save_result:
            np.savetxt('./result/train_loss.txt', np.array(train_loss), fmt="%f", delimiter=" ")
            np.savetxt('./result/train_acc.txt', np.array(train_acc), fmt="%f", delimiter=" ")
            if (X_valid is not None) and (y_valid is not None):
                np.savetxt('./result/valid_loss.txt', np.array(valid_loss), fmt="%f", delimiter=" ")
                np.savetxt('./result/valid_acc.txt', np.array(valid_acc), fmt="%f", delimiter=" ")
